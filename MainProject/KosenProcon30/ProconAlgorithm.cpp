...
void Procon30::ProconAlgorithm::initilize(const Game& game)
{
	...
	SafeConsole(U"ProconAlgorithm ビーム幅：", beam_size);
	...
	*/
	...
	for (nowSearchDepth = 0; nowSearchDepth < std::min(search_depth, game.MaxTurn - game.turn); nowSearchDepth++) {
		...
		if (nowBeamBucketArray.size() == 0) {
			SafeConsole(U"[努力の敗北] ProconAlgorithm 次状態が無いので打ち切ります。 GameID:", game.gameID);
	...
	if (search_depth != nowSearchDepth) {
		SafeConsole(U"ProconAlgorithm探索打ち切り深さ:", nowSearchDepth, U" 時間", game.turnTimer.ms());
	}
	else {
		SafeConsole(U"ProconAlgorithm探索終了時間", game.turnTimer.ms());
	}
